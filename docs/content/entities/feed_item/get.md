...
The `customer.feedItems.get()` method returns all fields for one FeedItem, as well as all other entities related to it. Note that this function is heavily rate-limited by Google, so avoid using it in production.
