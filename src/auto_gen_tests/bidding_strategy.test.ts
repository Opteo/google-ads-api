...
describe('BiddingStrategy', async () => {
    describe('reporting', async () => {
        it('can retrieve a list of BiddingStrategys with all fields (if valid)', async () => {
            const bidding_strategys = await customer.biddingStrategies.list()
            ...
            if (bidding_strategys.length > 0 && bidding_strategys[0].bidding_strategy.resource_name) {
                ...
                if (resource) {
                    const singleton = await customer.biddingStrategies.get(resource)
        ...
        it('throws an error when the request is invalid', async () => {
            await expect(
                customer.biddingStrategies.list({
                    limit: -10,
